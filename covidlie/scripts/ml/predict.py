...
# def predict(input_records, n, model_name, model, mis=None, mid=None, mis_vect=None, mx=None, model2=None):
#     """predict input sentences
#     input_records: {"id": id, "text": text}
#     n: int, number of misinformation
#     model_name: str, model name
#     mis: List[str], misinformation string list
#     mid: List[ind], misinformation id
#     mis_vect: ??
#     mx: ??
#     model2: model for some cases
#     """
#     # param: input_records: {"id": id, "text": text}, n: int, model_name: str, model2: second model
#     #        mis: List[str?], mid: List[int], mx: model._encode?


def predict_line(record, n, model_name, model, mis=None, mid=None, mis_vect=None, mx=None, model2=None):
    """predict input sentences
    ...
    print(record['id'])
    posts = [record['text']] * n
    post_ids = [record['id']] * n

    # Similarity Models
    # BoW Cosine
    if model_name == 'bow-cosine':
        corpus = mis + [record['text']]
    ...
    if model_name in ['bow-cosine', 'bert-base-cosine', 'bert-ft-cosine',
        ...
        post_vect = model._encode([record['text']])
    ...
    #     hits = searcher.search(record['text'], k = 86)
    #
    #     mid2 = []
    #     score = []
    ...
    if model_name in ['boe-log-snli', 'boe-log-mnli', 'boe-log-mednli']:
        post_vect = model._encode([record['text']]) * n
...
def read_csv_input(csv_path):
    ...
    df = pd.read_csv(csv_path)  # read lines
    assert "id" in df, f"no id in data {csv_path}"
    if "transcription" in df:
        df = df.rename(columns={"transcription": "text"})
    assert "text" in df, f"no text in data {csv_path}"
    return df
...
def main():
    ...
    # for jsonl input file path. file structure: {"id": id, "text": text}\n...
    parser.add_argument('--csv_input', type=str, required=False, help="Jsonl input file path")

    args = parser.parse_args()
    model_name = args.model_name
    db_input = args.db_input

    mis_vect, mx, model2 = None, None, None  # for predict function
    ...
    output = []
    if args.csv_input:
        data = read_csv_input(args.csv_input)
        for i in range(data.shape[0]):
            output.append(predict_line(data.loc[i], n, model_name, model, mis, mid, mis_vect, mx, model2))
        ...
        # output = []
        # output = predict(inputs, n, model_name, model, mis, mid, mis_vect, mx, model2)
        for record in inputs:
            output.append(predict_line(record, n, model_name, model, mis, mid, mis_vect, mx, model2))

        # # param: inputs: {"id": id, "text": text}, model_name: str, mis: List[str?], mid: List[??], mx: model._encode?,
        # for input in inputs:
        #     print (input['id'] )
        #     posts = [input['text']] * n
        #     post_ids = [input['id']] * n
